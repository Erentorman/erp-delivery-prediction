...
internal sealed record MockErpWorkOrder(string WorkOrderReference, string? OrderReference, string ProductReference, string Status, MockErpRouting Routing);
internal sealed record MockErpRouting(string RoutingReference, IReadOnlyList<MockErpOperation> Operations);
internal sealed record MockErpOperation(string OperationReference, int OperationSequence, string WorkCenterReference, long StandardDurationMinutes, IReadOnlyList<string> PredecessorOperationReferences);
internal sealed record MockErpCapacityAndCalendar(DateTimeOffset RangeStart, DateTimeOffset RangeEnd, IReadOnlyList<MockErpWorkCenter> WorkCenters, IReadOnlyList<MockErpWorkingShift> Shifts, IReadOnlyList<MockErpHoliday> Holidays, IReadOnlyList<MockErpPlannedDowntime> PlannedDowntimes);
internal sealed record MockErpWorkCenter(string WorkCenterRef, string Name);
